...
- [x] **1.10** Guardar en el duelo del torneo los campeones sorteados (`games[]`) *(hecho junto a la Fase 4, 2026-08-07)*: `registrarResultado`/`resolverLlave` reciben `campeones = {a:[ids], b:[ids]}` (la fila 2 del marcador) y con `construirGames()` llenan `duelo.games = [{ n, campeones:{a,b} }]`. Las 3 vistas pasan `$state.snapshot(jugadosA/B)` al registrar. Lo consume el **historial (4.2)**.

### 1.d — Lo "épico" (creativo, factible)
- [x] **1.11** **Sonido** *(hecho, 2026-08-07)*: **sintetizado con Web Audio** (sin archivos ni licencias — encaja con open-source/local-first/offline). `src/core/sonido.js`: `giro(durMs)` = ráfaga de tics que **decelera** con el frenado del tambor (tono cayendo); `revelacion()` = fanfarria (arpegio Do mayor + acorde con brillo) al aterrizar; `anotar()` = blip positivo al fijar el campeón en la fila 2; `clic()` disponible. `AudioContext` **perezoso** (1er gesto = girar). Enganchado en `DueloRuleta` (`DUR_GIRO` calcado del de `Ruleta`, reduced-motion ⇒ 900 ms). **Toggle de silencio** 🔊/🔇 en el shell (`App.svelte`), respaldado por `estado.ajustes.sonido` (persistido; `alternarSonido()` en el store). *Extensible*: se puede cambiar a SFX reales (CC0) por dentro sin tocar a quien llama. *(El banco de pruebas `CombateVista` queda mudo — el sonido vive en el combate del torneo.)*
- [x] **1.12** **Modo Overlay/OBS** *(hecho, 2026-08-07)*: botón 🖥️ en la cabecera del `ModalCombate` entra en modo overlay (`src/ui/overlay.svelte.js`). App se despoja del cromo (barra/contenido/pie ocultos) y el combate queda solo sobre el fondo elegido: **transparente** (OBS Browser Source respeta el alfa), **verde/magenta** (chroma key) o **negro**. El `<body>` toma su color de `html[data-overlay=…]` (en `app.css`); el `ModalCombate` se vuelve transparente y sin cromo. Clave: `src/ui/portal.js` (`use:portal`) reubica el modal en `<body>` para poder ocultar `.contenido` sin ocultar el combate. Control flotante `src/combate/OverlayControl.svelte` (esquina, discreto) para salir y cambiar el fondo; Escape también sale (sin cerrar el combate).
- [x] **1.13** **Tarjeta de resultado** *(hecho, 2026-08-07)*: `src/combate/tarjeta.js` dibuja con `<canvas>` nativo (sin librerías) una tarjeta 1200×675 (fondo Grieta + marco dorado, fichas con inicial, 👑 al ganador, marcador grande, VS, **tira de campeones jugados** con sus retratos, pie de marca) y la descarga como **PNG**. Retratos del mismo origen ⇒ canvas sin "taint" ⇒ `toBlob` OK; espera a las fuentes (Cinzel/Barlow) con fallback al sistema. Botón **📸 Tarjeta de resultado** integrado en `Marcador.svelte` (`exportable`/`titulo`), presente en ambas caras y en las 3 vistas. Usa el estado en vivo del combate, así que funciona ya sin esperar a 1.10.
...
- [x] **4.1** Modelo de **Temporada**: agrupar torneos *(hecho, 2026-08-07)*. Store: `crearTemporada`/`renombrarTemporada`/`borrarTemporada` (desvincula, no borra torneos) / `asignarTorneoATemporada(torneoId, temporadaId|null)` (mantiene en sync `torneo.temporadaId` ↔ `temporada.torneos[]`) / `cerrarTemporada`/`reabrirTemporada` (fija/limpia `fin`). `TemporadasVista.svelte` reescrita (antes placeholder): crear/renombrar/borrar temporadas, listar sus torneos (link a abrir, quitar, 🏆 campeón, estado), añadir torneos vía `<select>`, sección "Torneos sin temporada", cerrar/reabrir.
- [x] **4.2** Archivo/historial de torneos y duelos terminados *(hecho, 2026-08-07)*: `HistorialDuelos.svelte` (desplegable) lista los duelos terminados con fecha, invocadores, marcador, ganador/empate y la **tira de campeones jugados** de cada lado (de `duelo.games`, ver 1.10). Integrado en `TorneoDetalle` (todos los duelos del torneo, todas las etapas). Los torneos se archivan solos (persisten); las temporadas (4.1) los agrupan.
- [x] **4.3** **Estadísticas** *(hecho, 2026-08-07)*: motor puro `core/estadisticas.js` (`resumenEstadisticas(duelos)`), que reutiliza `clasificar`/`nemesis` de `clasificacion.js` y añade el uso de campeones (de `duelo.games`, 1.10). Produce clasificación resumida (winrate, V-E-D, mejor racha, invicto, campeón top por jugador) y los **destacados**: 🏆 más victorias · 💀 más derrotas · 🔥 mejor racha · 🛡️ invicto(s) · 📈 mejor winrate · 🎯 one-trick (más veces el mismo campeón) · 😈 némesis · ⭐ campeón más jugado. UI: `estadisticas/EstadisticasPanel.svelte` (desplegable reutilizable) montado **por torneo** en `TorneoDetalle` y **por temporada** en el nuevo `TemporadaDetalle`. Head-to-head disponible vía `headToHead()`.
- [x] **4.4a** **Tarjeta resumen de temporada** *(hecho, 2026-08-07)*: `temporadas/tarjetaTemporada.js` dibuja un PNG (canvas nativo, 1200×675) con podio, destacados y retratos de campeones; botón de descarga en `TemporadaDetalle`. Nuevo detalle de temporada con ruta `#/temporadas/<id>` (despacho por `ruta.param` en `TemporadasVista`, patrón de torneos) + botón "📊 Resumen" por temporada. Helpers de canvas compartidos extraídos a `ui/lienzo.js` (refactor de `combate/tarjeta.js`, mismo render).
- [ ] **4.4b** *(futuro)* Export `.json` de UNA temporada (respaldo/portabilidad acotada). El respaldo global (0.7, `core/export.js`) ya cubre el caso común; el valor real de esto exige un import selectivo con **merge** (colisiones de id, fusión vs. reemplazo), que queda pospuesto.
...
- **Fase 1 ⭐ — Sistema de Combate.** 1.1 ✅ (motor `Ruleta.svelte`), 1.2 ✅ (single/dual), 1.3 ✅ (retratos en la revelación), 1.4 ✅ (`PanelReglas.svelte`; Mirror → single/dual), **1.6 ✅ · 1.7 ✅** (puente Fase 1↔3: `DueloRuleta.svelte` = VS con retratos + ruleta según reglas), **1.8 ✅** (marcador de 2 filas `Marcador.svelte` + `SelectorCampeon.svelte`; fila 2 = campeones jugados en ambas caras), **1.9 ✅** (restock: la ruleta excluye los jugados), **1.11 ✅** (sonido sintetizado Web Audio + toggle 🔊/🔇). **1.11 ✅ · 1.12 ✅ · 1.13 ✅** cierran la Fase 1.d ("lo épico"). Con esto **3.7 queda cerrado** salvo la persistencia. **Fase 1 prácticamente completa**; solo quedan pendientes aparte: **1.10** guardar `games[]` *(diferido a la Fase 4)* y **1.5** presets de reglas. ← *siguiente sugerido: **Fase 4** (Temporadas & Registro, con 1.10 dentro), o **1.5** presets*
